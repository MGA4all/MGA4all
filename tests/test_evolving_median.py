...
def test_calculate_median_deployment_odd_history(asset_indices):
    ...
    history = [
        ...
        pd.Series([200, 200, 50], index=asset_indices),
        pd.Series([0, 300, 0], index=asset_indices),
...
def test_calculate_median_deployment_even_history(asset_indices):
    """Tests that calculate_median_deployment correctly averages the middle two values for an even-sized history."""
    history = [
        pd.Series([100, 200, 0], index=asset_indices),
        pd.Series([300, 500, 0], index=asset_indices),
...
def test_calculate_weights_evolving_median_basic_scenario(asset_indices):
    ...
    history = [
        pd.Series([900, 200, 50], index=asset_indices),
        pd.Series([200, 800, 150], index=asset_indices),
        pd.Series([700, 300, 100], index=asset_indices),
    ]
    ...
    actual = calculate_weights_evolving(
        latest_spore_mock, history, asset_indices, median_deployment
    )
    pd.testing.assert_series_equal(actual, expected)
...
def test_calculate_weights_evolving_median_robust_zero_median(asset_indices):
    ...
    history = [
        ...
        pd.Series([0, 0, 0], index=asset_indices),
        pd.Series([0, 0, 0], index=asset_indices),
...
def test_calculate_weights_evolving_median_latest_is_zero(asset_indices):
    ...
    history = [
        pd.Series([1000, 500, 800], index=asset_indices),
        pd.Series([200, 500, 200], index=asset_indices),
    ]
