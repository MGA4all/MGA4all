...
def compute_random_weights(deployed_capacity_series, target_techs):
    
    mga_weights_series = deployed_capacity_series 
    mga_weights_series[:] = np.round(np.random.uniform(-1, 1, len(mga_weights_series)), 2) 
...
def random_directions_algorithm(test_config, n_costopt):
    ...
    for j in range(1,test_config["alternatives"]+1):
        ...
        mga_weights_series = compute_random_weights(deployed_capacity_series, target_techs)
